...
`--sheet-rail-clear` is a `clamp()` written **in terms of the tokens**, never a pixel breakpoint: it reserves only the shortfall the *rail* needs, so on a wide window where the column already clears the rail it falls to `0`. It solves `chrome + inset + (panel − inset − toc − column) / 2 ≥ rail-open` for the inset, capped at the rail's own overhang. A hard-coded breakpoint there goes stale the moment the rail width, the column cap or the index width moves — which already happened once.

`--sheet-inset-left` is then `max(--sheet-rail-clear, --toc-reserve)`, and that `max()` is what **centres the reading column on the panel**. The two sides have to match for `margin: 0 auto` to centre anything on the panel rather than on a lopsided content box: reserving the rail's shortfall on the left (94px at 1710) and the index's gutter on the right (224px) put the column ~65px left of the panel's true centre. Taking the larger for both sides costs some column width at middling widths — at 1450 the column goes 986px → 922px — and buys a column actually centred on the space beside the rail. Both constraints still hold by construction: the left is never less than the rail needs, the right never less than the index does. `.sheet-head` inherits the fix for free because it cancels and repays `--sheet-inset-left` rather than the raw clamp — keep it referencing the token, not `--sheet-rail-clear`.
...
**Motion:** `--rail-motion` (the rail's box geometry — width, `flex-grow`, heights, paddings) and `--rail-item-motion` (the index rows' own fade + slide, `--motion-med` by default so the rows trail the box slightly). Both, plus `--rail-ease`, are remapped by `.app.sheet-closing`; that is the whole point of them being separate from `--motion-med`/`--ease-out`, which the remap cannot reach. Nothing in the rail should carry a literal duration — see the reduced-motion note above.
...
- **Rail reveal**: outside the one-time entry storyboard below and the sheet-open index state, the rail stays compact and expands only when the navigation is hovered or keyboard-focused (`:focus-within`). `.rail` transitions exactly two properties — `width` and `box-shadow`. The shadow belongs in that list because the sheet-open expanded rail is the only state that carries one; left out, it snapped on at hover-in against a 360ms width. `opacity` and `transform` were in the list for a long time and animated nothing: the rail is never faded or moved, it is `display: none` at 800px and under.
- **Rail index (`.rail-index`, sheet-open only)**: while a case study is open the rail becomes the index of case studies, which is what lets the feed column disappear and the sheet take the rest of the screen. `buildRailIndex()` builds it from the feed's own `.pin[data-page]` elements at load, so adding a card to the feed adds it here for free.
  ...
  - **Icons** (`RAIL_INDEX_ICONS`, keyed by sheet slug): one per study, **solid** on a 24×24 box — the route buttons above stay stroked, the studies read as a set of their own. They say what the project *is* (a cloud for a dream app, a flame for Habitat's streak), because the compact rail shows nothing else. Habitat was a target for a while: it read as "aim at a goal", which is every habit app, where the streak is the thing the product is actually built around. New marks are drawn to the set's metrics — solid, roughly y 2.4 to 21, about 4.8–19.2 across — so they carry the same ~2px padding and optical size; the flame keeps an inner notch because without it a solid flame reads as a teardrop at 22px. A study with no entry falls back to `default` rather than rendering an empty slot.
  ...
  - **The index enters from the right, travelling left** — the other half of the routes' tuck. `.rail-index-item` rests at `translateX(22px)` and the sheet-open rule takes it to `none`, so the studies arrive going the same direction the routes leave in and the two states read as one replacing the other. It rested at `-8px` for a long time, which slid the rows *rightward* into place, against the tuck — that opposition is what made the swap read as a crossfade rather than a hand-off. Closing reverses it for free: the rows animate back out to the right as they fade. The travel is deliberately short rather than matching the routes' full `--chrome-size` exit — the rail is auto-expanded to `--rail-open-w` while this plays, so there is no one "off the right edge" number, and 22px still carries the direction at either width while keeping the row inside the rail. `.rail-index` therefore sets `overflow-x: clip` explicitly: a lone `overflow-y: auto` computes the x axis to `auto` too, which would turn that offset into a scrollable region instead of clipping it.
- **Closing is one gesture.** The panel slides out on `--motion-slow` / `--ease-soft`; `.app.sheet-closing` retimes the rail's whole choreography to those same values (`--rail-motion` / `--rail-ease`), so the collapse and the slide are one move rather than two events at different speeds and curves. Two things go with it: `closeSheet()` removes `.is-sheet-expanded` **inside** the same rAF that adds `.sheet-closing` (outside it, the rail started collapsing a frame early on its own clock), and the `visibility` flip runs the collapse's full duration — on a shorter one the index blanked while its box was still shrinking, which was most of the chop. **The retiming has to reach the rows, not just the box.** `.rail-index-item`'s fade and slide sit on `--rail-item-motion` (`--motion-med` by default, `--motion-slow` while closing) and `--rail-ease`, so they leave on the panel's clock and curve. On `--motion-med`/`--ease-out` they sat outside the remap entirely: the rail box and the panel left at 480ms/ease-soft while the rows — the mass you actually watch leave — left at 420ms/ease-out. Anything new in the rail that animates on close belongs on one of the remapped tokens or it re-opens that seam.
- **One fade, never two stacked.** `.rail-index` animates geometry and `visibility` only — it has no opacity of its own. It used to fade `0 → 1` at the same time as every row inside it, so what you saw was the *product* of two curves: rows sat near-invisible well past their own delay and then arrived abruptly, which read as the index lagging the geometry it belongs to. The rows own the fade; the container owns the box. It is still fully hidden at rest without opacity — zero height from `flex: 0 1 0`, plus `visibility`. If you ever need to fade the index as a unit, take the fade off the rows first.
...
- **Sheet rail storyboard** (`playSheetRailEntry()`): on a device that can't hover (`hover: none`, 801px and up — tablets, touch laptops) the rail simply **stays expanded** while a study is open. There is no way to reopen it there: hover doesn't exist and tapping a row opens that study instead, so a collapsed index would strand the visitor with unlabelled icons. It reuses `.is-sheet-expanded`, and the panel already reserves the expanded width, so nothing else changes. Everywhere else, opening a study expands the rail for 2600ms so the rest of the work is visible without hunting, then collapses it to a column of card-colored swatches and hands the width back to the case study. Hover and keyboard focus reopen it from there — CSS `:hover` holds it open on its own, so the timer never has to know where the pointer is. Reduced motion skips the auto-expand and leaves the swatch column, which hover still opens. `closeSheet()` calls `endSheetRailEntry()`. **Switching studies replays the same beat** — `openSheet()`'s already-open branch calls `playSheetRailEntry()` too, so navigating from the index holds the rail a moment and then hands the width back, instead of collapsing the instant the pointer leaves.

  That beat only works because the click also gives focus away. `.rail-index-item` is a `<button>`, so a pointer click leaves focus on the row, and `.rail:focus-within` then pins the rail open permanently — measured at 240px 3.5s after a switch, with no hover and no `.is-sheet-expanded`; neither the timer nor the pointer leaving can outrank it. The row's handler moves focus to `#sheetClose` (the study's own way out, which is where a first open puts it) **only when `event.detail > 0`** — i.e. a real pointer. Keyboard activation keeps its place on purpose: holding the rail open is correct while someone is genuinely focused inside it, and yanking focus would strand a visitor browsing the index. Don't "simplify" that to an unconditional focus or a bare `blur()` — the first breaks keyboard browsing, the second drops focus on `<body>`.
